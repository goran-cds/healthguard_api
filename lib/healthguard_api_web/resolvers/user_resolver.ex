defmodule HealthguardApiWeb.Resolvers.UserResolver do
  ...
  def get_pacient_sensor_data_by_datetime(_, %{token: token, date: date}, _) do
    user = Users.get_user_by_token(token)
    ...
    case user.pacient_profile do
      ...
      _ ->
        if user.pacient_profile.sensor_data == [],
          do: {:ok, nil},
          else: {:ok, Users.get_pacient_sensor_data_by_datetime(user.pacient_profile.id, date)}
    end
  end
  ...
  def get_pacient_activities_by_date(_, %{token: token, date: date}, _) do
    user = Users.get_user_by_token(token)
    ...
    case user.pacient_profile do
      ...
      _ ->
        if user.pacient_profile.sensor_data == [],
          do: {:ok, nil},
          else: {:ok, Users.get_pacient_activities_by_date(user.pacient_profile.id, date)}
    end
  end
